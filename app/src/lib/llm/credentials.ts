...
function getEnvProviderKey(
  provider: LlmProvider
): { value?: string; envVarName?: string } {
  if (provider === "gemini") {
    const gemini = getEnv("GEMINI_API_KEY");
    if (gemini) {
      return { value: gemini, envVarName: "GEMINI_API_KEY" };
    }
    const google = getEnv("GOOGLE_API_KEY");
    if (google) {
      return { value: google, envVarName: "GOOGLE_API_KEY" };
    }
    return {};
  }
  if (provider === "openai") {
    const openai = getEnv("OPENAI_API_KEY");
    return openai ? { value: openai, envVarName: "OPENAI_API_KEY" } : {};
  }
  const anthropic = getEnv("ANTHROPIC_API_KEY");
  return anthropic ? { value: anthropic, envVarName: "ANTHROPIC_API_KEY" } : {};
}
...
export async function listLlmCredentialStatuses(): Promise<LlmCredentialStatus[]> {
  ...
  return llmProviders.map((provider) => {
    ...
    const envInfo = getEnvProviderKey(provider);
    if (envInfo.value) {
      return {
        provider,
        configured: true,
        source: "env" as const,
        maskedKey: maskApiKey(envInfo.value),
        envVarName: envInfo.envVarName ?? null,
      };
    }
...
export async function resolveRuntimeProviderKeys(): Promise<RuntimeProviderKeys> {
  ...
  return {
    gemini: stored.gemini ?? getEnvProviderKey("gemini").value,
    openai: stored.openai ?? getEnvProviderKey("openai").value,
    anthropic: stored.anthropic ?? getEnvProviderKey("anthropic").value,
  };
}
