...
type SlidesViewMode = "document" | "slides";
...
export default function Home() {
  ...
  const [slidesViewMode, setSlidesViewMode] =
    useState<SlidesViewMode>("document");
      ...
      <div className="flex items-center gap-3 border-b px-3 py-2">
        ...
        {hasSession && (
          <div className="flex items-center gap-1 rounded-md border bg-muted/40 p-1">
            <Button
              variant={slidesViewMode === "document" ? "default" : "ghost"}
              size="sm"
              className="h-8 px-3 text-xs"
              onClick={() => setSlidesViewMode("document")}
              aria-label="ドキュメントモード"
            >
              ドキュメント
            </Button>
            <Button
              variant={slidesViewMode === "slides" ? "default" : "ghost"}
              size="sm"
              className="h-8 px-3 text-xs"
              onClick={() => setSlidesViewMode("slides")}
              aria-label="スライドモード"
            >
              スライド
            </Button>
        ...
        {hasSession && slidesViewMode === "slides" ? (
          ...
          <ResizablePanelGroup orientation="horizontal">
            ...
            <ResizablePanel id="chat" defaultSize={38} minSize={25}>
              <ChatPanel artifactMode="document" />
            </ResizablePanel>
          </ResizablePanelGroup>
        )}
      </div>
