...
export function LlmCredentialsSettings({
  ...
  const storedProviders = useMemo(
    () => orderedProviders.filter((p) => p.source === "stored"),
    [orderedProviders]
  );
  const envProviders = useMemo(
    () => orderedProviders.filter((p) => p.source === "env"),
    [orderedProviders]
  );
    ...
    <div className={cn("flex h-full flex-col", className)}>
      <div className="space-y-1 p-3">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {`（保存済み ${storedProviders.length} / 環境変数 ${envProviders.length}）`}
        </p>
      </div>
      <Separator />
              ...
              <div key={status.provider} className="rounded-md border p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    {status.source === "stored"
                      ? "保存キーを使用中"
                      : status.source === "env"
                        ? `環境変数キーを使用中${status.envVarName ? ` (${status.envVarName})` : ""}`
                        : "未設定"}
                  </span>
                </div>
                ...
                {status.source === "env" && (
                  <p className="mt-2 text-[11px] text-muted-foreground">
                    このアプリ内に保存されたキーではありません。環境変数から検出しています。
                  </p>
                )}
